...
class RentSpider(scrapy.Spider):
    ...
    citys = [("北京", "bj"), ("天津", "tj"), ("上海", "sh"), ("重庆", "cq"), ("石家庄", "sjz"), ("太原", "ty"),
             ("西安", "xa"), ("济南", "jn"), ("郑州", "zj"), ("沈阳", "sy"), ("长春", "cc"), ("哈尔滨", "hrb"),
             ("南京", "nj"), ("杭州", "hz"), ("合肥", "hf"), ("南昌", "nc"), ("福州", "fz"), ("武汉", "wh"),
             ("长沙", "cs"), ("成都", "cd"), ("贵阳", "gy"), ("昆明", "km"), ("广州", "gz"), ("海口", "hk"),
             ("兰州", "lz"), ("呼和浩特", "hhht"), ("银川", "yc"),("宁波", "nb"), ("大连", "dl"), ("青岛", "qd"),
             ("无锡", "wx"), ("厦门", "xm"), ("烟台", "yt"), ("金华", "jh"),("温州", "wz"),("泉州", "quanzhou"),
    ...
    def parse(self,response):
        ...
        number = response.xpath('//span[@class="content__title--hl"]/text()').extract()[0]
        for each in all:

            price = each.xpath('span[@class="content__list--item-price"]/em/text()')

            location = each.xpath('p[@class="content__list--item--des"]/a/text()')

            # extract()经常使用来切片（脱壳）从一个对象中得到list

            # if title:
            #     item['title'] = title.extract()[0].replace('\t', '').replace('\n', '').replace(' ', '')
            ...
            # return 的作用：如果没有 return，则默认执行至函数完毕，返回的值一般是yeild的变量
            # yeild= return+记住返回位置           
            yield item
        nextlink = response.xpath('//div[@class="content__pg"]//@data-curpage').extract()
        total = int(response.xpath('//div[@class="content__pg"]//@data-totalpage').extract()[0])
